  ...
  return (
    <div className="grid grid-cols-1 gap-x-3 gap-y-4 px-4 pb-12 pt-5 min-[375px]:grid-cols-2 min-[415px]:grid-cols-3">
      {recipes.contents.map((recipe) => (
        <div key={recipe.id} className={`relative w-full ${styles.resultGrid}`}>
          <Link
            href={{
          ...
          <p
            className={`${styles.resultTags} box-border flex items-center gap-1 rounded-2xl bg-black/60 px-2 py-[2px] leading-4 text-white`}
          >
